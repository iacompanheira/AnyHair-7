import { Service, Professional, Appointment, AdminSettings, User, Admin } from './types';
import { SERVICES, PROFESSIONALS, APPOINTMENTS, INITIAL_ADMIN_SETTINGS, CLIENTS, ADMINS } from './constants';
...
export const deleteService = async (id: string): Promise<void> => {
    await delay(SIMULATED_DELAY);
    // Remove the service from the main list
    services = services.filter(s => s.id !== id);
    
    // Also remove this service from any professional's specialties
    professionals = professionals.map(prof => {
        if (prof.specialties.includes(id)) {
            return {
                ...prof,
                specialties: prof.specialties.filter(specId => specId !== id)
            };
        }
        return prof;
    });
...
export const saveAdmin = async (admin: Admin): Promise<Admin> => {
    await delay(SIMULATED_DELAY);
    // Note: In a real app, password handling would be secure.
    if (admin.id) {
        admins = admins.map(a => a.id === admin.id ? admin : a);
    } else {
