...
export const AdminClients: React.FC<AdminClientsProps> = ({ onViewClient }) => {
    ...
    return (
        <div>
                                            ...
                                            <tr key={client.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => onViewClient(client.id)}>
                                                ...
                                                <td className="p-2">
                                                    <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                                                        ...
                                                        <Button variant="ghost" className="text-red-600 text-base border border-gray-200 shadow-none rounded-md px-2 py-0.5" onClick={() => openDeleteConfirmation(client.id)} isLoading={deletingId === client.id}>{deletingId === client.id ? '' : 'Excluir'}</Button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
            ...
            >
                <p>Tem certeza que deseja excluir este cliente? Todos os seus agendamentos e histórico serão perdidos. Esta ação não pode ser desfeita.</p>
            </ConfirmationModal>
        </div>
    );
};