...
export const AdminProfessionals: React.FC = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        setDeletingId(professionalToDelete);
        setIsConfirmModalOpen(false);
        try {
            await api.deleteProfessional(professionalToDelete);
            setProfessionals(professionals.filter(p => p.id !== professionalToDelete));
    ...
    return (
        <div>
                                            ...
                                            <tr key={prof.id} className="border-b hover:bg-gray-50">
                                                ...
                                                <td className="p-2">
                                                    <div className="flex gap-2">
                                                        ...
                                                        <Button variant="ghost" className="text-red-600 text-base border border-gray-200 shadow-none rounded-md px-2 py-0.5" onClick={() => openDeleteConfirmation(prof.id)} isLoading={deletingId === prof.id}>{deletingId === prof.id ? '' : 'Excluir'}</Button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
            ...
            >
                <p>Tem certeza que deseja excluir este profissional? Todos os agendamentos associados a ele precisarão ser remanejados. Esta ação não pode ser desfeita.</p>
            </ConfirmationModal>
        </div>
    );
};