...
export const MainScreen: React.FC<MainScreenProps> = ({ userRole, onLogout }) => {
    ...
    const renderContent = () => {
        ...
        switch (currentView) {
            ...
            case 'admins':
                return <AdminAdmins />;
            case 'settings_hub':
                return <AdminSettingsHub onNavigate={handleLinkClick} />;
            default:
                return <AppointmentsList role={userRole} />;
        }
    };
    ...
    const adminMenu = [
        ...
        { view: 'settings_hub', icon: 'fa-cog', label: 'Configurações' },
    ...
    const isSettingsActive = ['settings_hub', 'services_admin', 'professionals', 'admins', 'settings_admin', 'schedule', 'reports'].includes(currentView);
                ...
                <nav className="flex-grow space-y-2">
                    {menuItems.map(item => (
                        <SidebarLink 
                            ...
                            icon={item.icon} 
                            label={item.label}
                            isActive={item.view === 'settings_hub' ? isSettingsActive : currentView === item.view} 
                            ...
                            {menuItems.map(item => (
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${(item.view === 'settings_hub' ? isSettingsActive : currentView === item.view)
