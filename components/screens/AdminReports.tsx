...
const REPORTS_FILTER_KEY = 'anyhair-reports-date-filter';
const filters = ['Tudo', '30 Dias', '90 Dias', 'Último Ano'];

export const AdminReports: React.FC = () => {
    const [activeFilter, setActiveFilter] = useState(() => {
        const savedFilter = localStorage.getItem(REPORTS_FILTER_KEY);
        return savedFilter && filters.includes(savedFilter) ? savedFilter : 'Tudo';
    });

    useEffect(() => {
        localStorage.setItem(REPORTS_FILTER_KEY, activeFilter);
    }, [activeFilter]);
