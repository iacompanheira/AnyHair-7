...
import { Professional, Appointment, Service, AdminSettings } from '../../types';
...
export const AdminCalendar: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [profsData, apptsData, servicesData, settingsData] = await Promise.all([
                        ...
                        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                            <div className="grid grid-cols-[60px_repeat(7,1fr)] text-center font-bold text-base text-gray-600 border-b">
                                ...
                                {weekDays.map(day => {
                                    ...
                                    const isWorkingDay = settings?.workingDays.includes(dayIndex);
                                    return (
                                     <div key={day.toISOString()} className={`p-2 border-r 
                                        ${new Date().toDateString() === day.toDateString() ? 'bg-pink-100' : ''}
                                 ...
                                 {timeLabels.map((time, timeIndex) => (
                                   <React.Fragment key={time}>
                                        ...
                                        {weekDays.map((day, dayIndex) => {
                                            ...
                                            const isWorkingDay = settings?.workingDays.includes(dayOfWeekIndex);
                                            ...
                                            return (
                                                <div key={`${time}-${dayIndex}`} className={`border-r border-b p-0.5 ${!isWorkingDay ? 'bg-gray-100 cursor-not-allowed' : ''}`} style={{gridRow: timeIndex+1, gridColumn: dayIndex+2}}>
